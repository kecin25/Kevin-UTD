...
function doSearchResult(input) {
    ...
    xhr.open('GET', 'http://api.marketstack.com/v1/tickers?search=' + input + '&access_key=' + API_KEY);
    xhr.onload = () => {
...
function createAddButton(stock) {
    ...
    button.onclick = () => {
        ...
        if(stockNames.includes(stock.symbol)) {
            ...
            xhr.open('GET', 'http://api.marketstack.com/v1/eod?symbols=' + stock.symbol + '&access_key=' + API_KEY);
            xhr.onload = () => {
...
function getNavBarInfo() {
    ...
    for(let i = 0; i < 4; i++) {
        setTimeout(() => {
            ...
            xhr.open('GET', 'http://api.marketstack.com/v1/eod?symbols=' + symbol + '&access_key=' + API_KEY);
            xhr.onload = () => {
